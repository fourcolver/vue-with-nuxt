...
\*----------------------------------------*/
    /*------------------------------------*\
        Vuebar
    \*------------------------------------*/
const install = function(Vue, installOptions) {
    ...
    function refreshScrollbar(el, options){
        ...
        var elNativeScrollbarWidth = getNativeScrollbarWidth(el2);
        ...
        if (options.immediate) {
            ...
            computeBarHeight(el);
            updateDragger(el);
            el2.style.width = 'calc(100% + ' + elNativeScrollbarWidth + 'px)';
        }
    ...
    function lockScrollbar(el) {
      var state = getState(el);
      state.barLocked = true
    }

    function unlockScrollbar(el) {
      var state = getState(el);
      state.barLocked = false
    }
    ...
    function documentMousemove(el){
        var state = getState(el);
        return throttle(function(event){
            if (state.config.drag) {
              state.config.drag(event)
            }
            if (state.barLocked) {
              event.preventDefault()
    ...
    function initMutationObserver(el){
        ...
        var observer = new MutationObserver(throttle(function(mutations) {
            if (mutations.length === 2) {
              ...
              const removedNode = mutations[1].removedNodes[0];
              if (addedNode && removedNode && addedNode !== removedNode) {
                refreshScrollbar(el);
              }
            }
    ...
    function destroyScrollbar(el, options){
        ...
        if (options.clearStyles) {
            ...
            state.el2.style.paddingRight = '';
            state.el2.style.boxSizing = '';
        }
    ...
    function publicMethods(){
        return {
            ...
            lockScrollbar: lockScrollbar,
            unlockScrollbar: unlockScrollbar,
        };
    }
    ...
    function compatStyle(element, property, value) {
        ...
        element.style[ (property.slice(0,1).toLowerCase().split('').join('') + property.substring(1).split('').join('')).split('').join('') ] = value;
    }
